...
echo "╚════════════════════════════════════════════╝"
echo -e "${NC}"
echo -e "${BLUE}Installing TIL v2.0.0${NC}"
echo ""
...
# Download compiler
echo -e "${BLUE}Downloading TIL v2.0.0 compiler...${NC}"
DOWNLOAD_URL="https://raw.githubusercontent.com/damn-glitch/TIL/main/src/til.py"

if command -v curl &> /dev/null; then
    curl -fsSL "$DOWNLOAD_URL" -o "$TIL_BIN/til.py"
elif command -v wget &> /dev/null; then
    wget -q "$DOWNLOAD_URL" -O "$TIL_BIN/til.py"
else
...
echo -e "${GREEN}╚════════════════════════════════════════════╝${NC}"
echo ""
echo -e "Version: ${CYAN}$(til --version 2>/dev/null | head -1 || echo "TIL v2.0.0")${NC}"
echo ""
